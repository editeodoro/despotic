...
class hot_wind_table_data(object):
    ...
    def uh_lim(self, yidx, midx):
        ...
        if self.__data[ystr][mstr] is None:
            self.read(yidx, midx)
        if not self.__data[ystr][mstr]:
            raise IOError("pwind: unable to import data for hot winds "
                          "case y = {:d}, m = {:d}; have you run the "
                          "make_hot_tab.py script?".format(yidx, midx))
        lim = np.zeros(2)
...
class pwind(object):
    ...
    def s_crit(self, varpi, varpi_t, u=0.0):
        ...
        s_crit.resize(ns, refcheck=False)
        return s_crit
    ...
    def a_crit(self, varpi, varpi_t, u=0.0):
        ...
        a_crit.resize(na, refcheck=False)
        return a_crit
